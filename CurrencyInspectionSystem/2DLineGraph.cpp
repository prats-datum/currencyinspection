...
void C2DLineGraph::BuildGraph(HDC hDC)
{
	...
	int seg_area = int(m_Size.cx*0.8);
	...
	int startX = m_Position.x + int( m_Size.cx*0.04 );
	int startY = m_Position.y + int( m_Size.cy*0.95 );
	// Draw x coordinate axis
	::MoveToEx( hDC, startX, startY, NULL );
	::LineTo( hDC, m_Size.cx * 0.95, startY );
	// Draw x coordinate axis arrow
	::MoveToEx( hDC, (m_Size.cx * 0.95), startY, NULL );
	::LineTo( hDC, int((m_Size.cx * 0.95)*0.98), startY-int(m_Size.cy*0.02) );
	::MoveToEx( hDC, (m_Size.cx * 0.95), startY, NULL );
	::LineTo( hDC, int((m_Size.cx * 0.95) *0.98), startY+int(m_Size.cy*0.02) );
	...
	::LineTo( hDC, startX - int((m_Size.cx *0.95)*0.015), m_Position.y + int(m_Size.cy*0.085) );
	::MoveToEx( hDC, startX, m_Position.y + int(m_Size.cy*0.05), NULL );
	::LineTo( hDC, startX + int((m_Size.cx *0.95)*0.015), m_Position.y + int(m_Size.cy*0.085) );
					...
					while ( curr_ser != NULL )
					{
						...
						if ( curr_seg->index > m_DrawSegmentsCompleted )
						{
							...
							while ( next_val != NULL )
							{
								...
								dist = (double(seg_perc)/double(m_SegmentsPercent))*sqrt(pow((val_start.x-val_end.x),2.0)+pow((val_start.y-val_end.y),2.0));
	...
	else if ( m_Animation == FALSE )
	{
		...
		if ( ( m_SegmentsNumber > 1 ) && ( m_SeriesNumber > 0 ) )
		{
			...
			while ( curr_seg != NULL )
			{
				...
				//m_SegmentFont->CreateFont( font_height, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
				//	DEFAULT_CHARSET, OUT_CHARACTER_PRECIS, CLIP_CHARACTER_PRECIS, PROOF_QUALITY, DEFAULT_PITCH | FF_DONTCARE, _T("Arial") );
				m_SegmentFont->CreateFont(10,                            // Height
					0,                             // Width
					0,                             // Escapement
					0,                             // Orientation
					FW_BOLD,                       // Weight
...
void C2DLineGraph::BuildGraphLegend(HDC hDC)
{
	...
	CPoint start = CPoint( m_Position.x + int(m_Size.cx*0.93), m_Position.y + int(m_Size.cy*0.014) + offset );
	CSize sz;
	CString str = _T("");
	CRect rect;
	CBrush* rBrush = NULL;
	...
	::Rectangle( hDC, m_Position.x + int(m_Size.cx*0.9), m_Position.y + offset, m_Position.x + int((m_Size.cx * 0.9)*1.25), m_Position.y + int((m_SeriesNumber+0.5)*ts.cy + offset) );
