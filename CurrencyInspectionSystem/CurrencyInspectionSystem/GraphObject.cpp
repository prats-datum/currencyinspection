...
CGraphObject::CGraphObject()
{
	...
	m_GraphaxistitleColor = RGB(0,0,0);
	m_GraphSubaxistitleColor = RGB(0,0,0);
...
BOOL CGraphObject::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
	...
	m_GraphTitleFont->CreateFont(18,                            // Height
		0,                             // Width
		0,                             // Escapement
		0,                             // Orientation
		FW_BOLD,                       // Weight
		...
		_T("Microsoft Sans Serif"));                     // Facename
...
void CGraphObject::CreateGraph(int g_type)
{
	...
	switch ( m_GraphType )
	{
		...
		case GT_2DBAR:
			{
				// Set graph size and position
				CPoint g_point = CPoint( int(m_Size.cx*0.05), int(m_Size.cy*0.2) );
				CSize g_size = CSize( int(m_Size.cx*0.8), int(m_Size.cy*0.7) );
		...
		case GT_2DPIE:
			{
				// Set graph size and position
				CPoint g_point = CPoint( int(m_Size.cx*0.07), int(m_Size.cy*0.2) );
				CSize g_size = CSize( int(m_Size.cx*0.5), int(m_Size.cy*0.7) );
...
void CGraphObject::OnPaint() 
{
	...
	if ( m_GraphFillType == GB_GRADIENT )
	{
		...
		oldTextColor = m_GraphDC->SetTextColor( m_GraphaxistitleColor );
		...
		oldTextColor = m_GraphDC->SetTextColor( m_GraphSubaxistitleColor );
	...
	if (m_GraphType != GT_2DPIE)
	{
		::MoveToEx(m_GraphDC->m_hDC, 0, m_Size.cy - 10, NULL);
		::LineTo(m_GraphDC->m_hDC, m_Size.cx, m_Size.cy - 10);
	}
...
void CGraphObject::SetGraphaxistitleColor(COLORREF t_color)
{
	// Set graph title color
	m_GraphaxistitleColor = t_color;
}

void CGraphObject::SetGraphSubaxistitleColor(COLORREF s_color)
{
	// Set graph subtitle color
	m_GraphSubaxistitleColor = s_color;
}

COLORREF CGraphObject::GetGraphaxistitleColor()
{
	return m_GraphaxistitleColor;
}

COLORREF CGraphObject::GetGraphSubaxistitleColor()
{
	return m_GraphSubaxistitleColor;
}
