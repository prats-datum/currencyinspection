...
void CViewPortControl::setViewer(CChartViewer *viewer)
{
	...
	if (0 != m_EnergyChartViewer)
		m_EnergyChartViewer->setViewPortControl(0);
	m_EnergyChartViewer = viewer;
	setViewPortManager(viewer);
	if (0 != m_EnergyChartViewer)
		m_EnergyChartViewer->setViewPortControl(this);
...
void CViewPortControl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	// Mouse wheel events are only sent to the control in focus. So if mouse wheel zooming is 
	// enabled, we must get the focus in order to receive mouse wheel events.
	if ((0 != m_EnergyChartViewer) && (m_EnergyChartViewer->getMouseWheelZoomRatio() != 1)
...
BOOL CViewPortControl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	// Process the mouse wheel only if the mouse is over the plot area
	ScreenToClient(&pt);
	if ((0 == m_EnergyChartViewer) || (!isOnPlotArea(toImageX(pt.x), toImageY(pt.y))))
		return FALSE;

	// Ask the CChartViewer to zoom around the center of the chart
	int x = m_EnergyChartViewer->getPlotAreaLeft() + m_EnergyChartViewer->getPlotAreaWidth() / 2;
	int y = m_EnergyChartViewer->getPlotAreaTop() + m_EnergyChartViewer->getPlotAreaHeight() / 2;
	return m_EnergyChartViewer->onMouseWheelZoom(x, y, zDelta);
}
